...
def create_task(
    ...
    # Get the created task - it's the most recent one in this project
    tasks = service.get_tasks_by_project(project_id)
    # The newest task will be the last one since they're ordered by created_at
...
def delete_task(
    ...
    """Delete a task."""
    # First verify the task belongs to the project
    task = service.task_repository.get_by_id(task_id)
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    # Delete the task using repository directly
